...
func TestReadOnlyConnection(t *testing.T) {
	...
	container, err := testcontainers.GenericContainer(t.Context(), testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			Image:        "mysql:8.0",
			Env:          map[string]string{"MYSQL_ROOT_PASSWORD": "root-test", "MYSQL_USER": "test", "MYSQL_PASSWORD": "test", "MYSQL_DATABASE": "testdb"},
			ExposedPorts: []string{"3306/tcp"},
			WaitingFor: wait.ForSQL("3306/tcp", "mysql", func(host, port string) string {
				return "test:test@tcp(" + net.JoinHostPort(host, strings.TrimSuffix(port, "/tcp")) + ")/testdb"
...
`, host, port.Port(), host, port.Port())
	...
	for _, sql := range []string{
		...
		"SET SESSION transaction_read_only=0; INSERT INTO readonly_rows VALUES (3)",
	} {
		output, err := run("readonly", sql)
		require.Error(t, err, "%s: %s", sql, output)
		require.Contains(t, strings.ToLower(string(output)), "error 1064", "%s", sql)
		assertRows("readonly", [][]int{{1}, {2}})
		assertRows("writable", [][]int{{1}, {2}})
	}
