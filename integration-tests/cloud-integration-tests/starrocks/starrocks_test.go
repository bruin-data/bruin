...
const connectionName = "starrocks-default"

func TestStarRocksWorkflows(t *testing.T) {
	testcontainers.SkipIfProviderIsNotHealthy(t)
	if runtime.GOOS != "linux" && os.Getenv("STARROCKS_RUN_ON_NON_LINUX") == "" {
		t.Skip("StarRocks all-in-one container requires vm.max_map_count >= 2000000 in the Docker VM; on macOS run `docker run --rm --privileged alpine sysctl -w vm.max_map_count=2000000`, then set STARROCKS_RUN_ON_NON_LINUX=1 to force")
	}

	ctx := context.Background()
	image := os.Getenv("STARROCKS_TEST_IMAGE")
	if image == "" {
	...
	require.NoError(t, err, "failed to start the StarRocks container")
	t.Cleanup(func() {
	...
	waitForStarRocksSQL(t, host, mappedPort.Port())
	...
	tests := []struct {
	...
	}{
		{
			...
			workflow: e2e.Workflow{
				...
				Steps: []e2e.Task{
					{
						Name:    "ping the StarRocks connection",
						Command: binary,
		...
		},
		{
			...
			workflow: e2e.Workflow{
				Name: "merge-materialization",
				Steps: []e2e.Task{
					runAsset("merge: create primary key table with full refresh", "merge-initial-pipeline/assets/accounts.sql"),
					queryContains(
						"merge: verify primary key table model",
						"SHOW CREATE TABLE `bruin_test`.`merge_accounts`",
						"PRIMARY KEY(`account_id`)",
					),
					runAssetWithArgs("merge: run primary key upsert", "merge-updated-pipeline/assets/accounts.sql"),
					queryContains(
					...
					queryContains(
						...
						"SELECT account_status, balance FROM `bruin_test`.`merge_accounts` WHERE account_id = 2",
						"updated", "30",
					),
					queryContains(
						"merge: verify unchanged row",
						"SELECT account_status, balance FROM `bruin_test`.`merge_accounts` WHERE account_id = 1",
						"kept", "10",
					),
					queryContains(
						"merge: verify inserted row",
						"SELECT account_status, balance FROM `bruin_test`.`merge_accounts` WHERE account_id = 3",
						"new", "40",
					),
				},
			},
		...
		},
		{
			...
			workflow: e2e.Workflow{
				Name: "source-asset",
				Steps: []e2e.Task{
					runAsset("run StarRocks source metadata asset", "source-pipeline/assets/source.asset.yml"),
				},
			},
		},
	}
...
func waitForStarRocksSQL(t *testing.T, host, port string) {
	t.Helper()
	...
	for time.Now().Before(deadline) {
		...
		if err == nil {
			...
			aliveBackend, err = hasAliveStarRocksBackend(ctx, db)
	...
	t.Fatalf("StarRocks SQL endpoint did not become ready: %v", lastErr)
}

func hasAliveStarRocksBackend(ctx context.Context, db *sql.DB) (bool, error) {
	...
	return false, fmt.Errorf("no alive StarRocks backend reported by SHOW BACKENDS")
}
