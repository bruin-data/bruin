				...
				Steps: []e2e.Task{
					{
						...
						Command: binary,
						Args:    append(append([]string{"query"}, configFlags...), "--env", "default", "--query", "DROP TABLE IF EXISTS menu;"),
						Env:     []string{},
						Expected: e2e.Output{
							ExitCode: 1,
						},
					...
					},
					{
						...
						Command: binary,
						Args:    append(append([]string{"query"}, configFlags...), "--connection", "athena-default", "--query", "SELECT ID, Name, Price, _is_current FROM menu ORDER BY ID, _valid_from;", "--output", "csv"),
						Env:     []string{},
						Expected: e2e.Output{
							ExitCode: 0,
					...
					},
					{
						...
						Command: binary,
						Args:    append(append([]string{"query"}, configFlags...), "--env", "athena-default", "--query", "SELECT * FROM menu;"),
						Env:     []string{},
						Expected: e2e.Output{
			...
			workflow: e2e.Workflow{
				...
				Steps: []e2e.Task{
					...
					},
					{
						...
						Expected: e2e.Output{
							ExitCode: 0,
							Contains: []string{"Finished: products"},
						},
						Asserts: []func(*e2e.Task) error{
							e2e.AssertByExitCode,
							e2e.AssertByContains,
					...
					},
					{
						...
						Command: binary,
						Args:    append(append([]string{"query"}, configFlags...), "--connection", "athena-default", "--query", "SELECT product_id,product_name,stock,_is_current,_valid_from FROM products ORDER BY product_id, _valid_from;", "--output", "csv"),
						Env:     []string{},
						Expected: e2e.Output{
							ExitCode: 0,
