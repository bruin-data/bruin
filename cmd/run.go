...
func Run(isDebug *bool) *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			// Auto-configure PostgreSQL CDC sources: enable gong and set merge strategy
					...
					if asset.Type == pipeline.AssetTypeIngestr {
						...
						for _, pg := range cm.SelectedEnvironment.Connections.Postgres {
							if pg.GetName() == sourceConn && pg.CDC {
								// PostgreSQL CDC detected, set incremental_strategy to merge
								asset.Parameters["incremental_strategy"] = "merge"

								// Enable gong if not already enabled
								if !runConfig.UseGong {
