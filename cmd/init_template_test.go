...
func TestInitSelfHealDemoCopiesPipelineTemplate(t *testing.T) {
	...
	err = Init().Run(t.Context(), []string{"init", "demo-self-heal-pipeline"})
	require.NoError(t, err)

	pipelineRoot := filepath.Join(targetRoot, "demo-self-heal-pipeline")
	...
	require.NoError(t, err)
	require.Contains(t, string(configContent), "name: demo-self-heal-pipeline")
	require.Contains(t, string(configContent), "path: demo-self-heal-pipeline.duckdb")
}
...
func TestSelfHealDemoTemplateContainsDataProblemScenarios(t *testing.T) {
	...
	seedPipeline := readTemplate("demo-self-heal-pipeline/demo-seed/pipeline.yml")
	require.Contains(t, seedPipeline, "name: demo-seed")

	demoPipeline := readTemplate("demo-self-heal-pipeline/demo-pipeline/pipeline.yml")
	require.Contains(t, demoPipeline, "name: demo-pipeline")

	seed := readTemplate("demo-self-heal-pipeline/demo-seed/assets/order_status_history.asset.yml")
	require.Contains(t, seed, "name: raw.order_status_history")
	require.Contains(t, seed, "type: duckdb.seed")

	duplicate := readTemplate("demo-self-heal-pipeline/demo-pipeline/assets/status_snapshot.sql")
	...
	quality := readTemplate("demo-self-heal-pipeline/demo-pipeline/assets/order_margin.sql")
	...
	freshness := readTemplate("demo-self-heal-pipeline/demo-pipeline/assets/daily_activity.sql")
	...
	schemaDrift := readTemplate("demo-self-heal-pipeline/demo-pipeline/assets/product_prices.sql")
	...
	templatesRoot, err := templates.Templates.ReadDir("demo-self-heal-pipeline/demo-pipeline/assets")
	require.NoError(t, err)
