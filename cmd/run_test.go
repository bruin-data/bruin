...
func TestScriptAssetsWithUnsupportedHooks(t *testing.T) {
	t.Parallel()
	...
	assets := []*pipeline.Asset{
		...
		{
			Name: "python_without_hooks",
			Type: pipeline.AssetTypePython,
		},
		{
			Name: "python_default_only",
			Type: pipeline.AssetTypePython,
			Hooks: pipeline.Hooks{
				Pre:  []pipeline.Hook{{Query: "select default pre"}},
				Post: []pipeline.Hook{{Query: "select default post"}},
		...
		},
		{
			...
			Type: pipeline.AssetTypeBigqueryQuery,
			Hooks: pipeline.Hooks{
				Pre: []pipeline.Hook{{Query: "select 3"}},
			},
		},
		nil,
	}

	got := scriptAssetsWithUnsupportedHooks(assets, p)
	assert.Equal(t, []string{"python_with_hooks", "r_with_hooks"}, got)
}
