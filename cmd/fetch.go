...
func applySchemaPrefix(ctx context.Context, queryStr, dialect string, parser *sqlparser.SQLParser, pipelineInfo *ppInfo, conn interface{}) (string, error) {
	if dialect == "" {
	...
	if err != nil {
		// If we can't parse tables, return the original query
		return queryStr, nil
	}
	...
	dbFetcherConn, ok := conn.(interface {
		GetDatabaseSummary(ctx context.Context) (*query.QueryResult, error)
	})
	_ = dbFetcherConn // We'll check if tables exist if this is available
