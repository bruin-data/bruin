...
## Task
Run the full pipeline with `bruin run --environment dev pipeline` and inspect the dev database. Record the destination names and a read-only `bruin data-diff` comparison plan in `docs/dev-run.md`. Do not write to the default environment during this task.

## Rubric (for `review my work`)
- [ ] The dev run completes every non-broken asset in the prefixed dev destination.
