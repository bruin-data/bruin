...
- `fct_order_lines` carries a relationships check without declaring the raw `products` dependency;
  unmatched source keys remain auditable in `source_product_id` while the nullable dimension key
  prevents the initial run from adding a second blocking failure.
