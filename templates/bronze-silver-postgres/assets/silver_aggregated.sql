...
type: pg.sql
description: |
  Aggregates the bronze Frankfurter exchange rate feed into a silver summary table with fresh rates, rolling averages, and observation counts for downstream analytics.
...
latest AS (
    SELECT DISTINCT ON (currency_code, base_currency)
        ...
        rate AS latest_rate
    FROM bronze_data
    ORDER BY currency_code ASC, base_currency ASC, rate_date DESC
)
