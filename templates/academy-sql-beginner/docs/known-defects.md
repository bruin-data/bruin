...
- **Lesson (Step 4):** a filter written as `WHERE order_status != 'cancelled'`
...
- **Lesson (Step 5):** `AVG(unit_cost)` divides by the number of non-NULL values,
...
- **Lesson (Step 6 and capstone):** an `INNER JOIN` to `products` silently
...
- **Lesson (capstone):** joining `orders` to `customers` on
  `customer_id` repeats those customers' orders, inflating any `SUM` over the join.
  A `DISTINCT` or `COUNT(DISTINCT ...)` can appear to fix the count while a
  `SUM(order_total)` beside it stays wrong. The real fix is to aggregate before
  joining, or join to a de-duplicated customer list.
