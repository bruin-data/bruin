/* @bruin
name: chargebee_reports.monthly_mrr_movements
type: bq.sql
description: >
  Customer-level MRR movement ledger. Movements are classified after summing all
  subscriptions for a billing customer in a currency, so an internal plan swap
  does not appear as separate churn and new-business events. Each customer month
  is classified as new, reactivation, expansion, contraction, churn, or
  no_change, with amounts that always roll forward from beginning to ending MRR.

materialization:
  type: table

depends:
  - chargebee_reports.monthly_mrr_by_customer
...
columns:
  ...
  - name: currency_code
    type: STRING
    description: Native currency. Movements are classified per currency and never converted.
    primary_key: true
    checks:
      - name: not_null
  - name: company_name
  ...
  - name: previous_observed_month
    type: DATE
    description: >
      Previous month in which this customer and currency were observed. Null in
      the customer's first observed month.
  ...
  - name: beginning_mrr_minor
    type: NUMERIC
    description: MRR carried in from the previous observed month, zero when none exists.
  ...
  - name: net_mrr_change_minor
    type: NUMERIC
    description: >
      Ending less beginning MRR. Null when the month cannot be reconciled
  ...
  - name: movement_type
    type: STRING
    description: >
      Classified movement, one of new, reactivation, expansion, contraction,
      churn, no_change, first_observed_month, or snapshot_gap. The last two mark
      months that cannot be reconciled.
    checks:
      - name: not_null
...
WITH customer_mrr_history AS (
  ...
  WINDOW customer_window AS (
    PARTITION BY chargebee_customer_id, currency_code
