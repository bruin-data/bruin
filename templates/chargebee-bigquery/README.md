...
MRR / ARR, retention, MRR bridge movements, customer mix, plan packaging, invoice
collections, and account follow-up. All tabs are filterable by currency and date
range. The Overview tab pairs the MRR trajectory with a latest-month waterfall,
growth quality (MRR versus logo growth), and a movement-driver table that maps each
driver to a suggested action. The Retention tab uses a cohort heatmap, a clear
additions-versus-losses comparison, and account-level movement details. The
Monetization tab uses complementary customer-count and revenue-mix pies by MRR band;
plan-level detail remains in the table. Collections includes cash conversion and
MRR exposure by payment urgency. Preview it locally:
