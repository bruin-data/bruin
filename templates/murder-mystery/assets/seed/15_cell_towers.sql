...
description: |
  The 58 mobile cell sites covering Yorkville, laid out on the town's local
...
SELECT
    ...
    CASE n
        WHEN 28 THEN 'Wychwood Square and the civic steps; carries the heaviest daytime load in the town'
        WHEN 36 THEN 'operator microcell on the Loma House roof, serving the north block only'
        WHEN 20 THEN 'Macpherson Mews and the service yards behind the square'
        WHEN 35 THEN 'Davenport Road, the northern approach and the freight weighbridge'
        WHEN 12 THEN 'Russell Hill Road, Yorkville General Clinic and the ambulance apron'
        WHEN 43 THEN 'Nordheimer Rise, the Bracondale ranges, reservoir path and allotments'
