...
(
    select 
        ...
        array_agg(distinct p.value.string_value ignore nulls limit 3) as example_string_values,
        array_agg(distinct p.value.int_value ignore nulls limit 3) as example_int_values,
        array_agg(distinct p.value.float_value ignore nulls limit 3) as example_float_values,
        string_agg(distinct event_name order by event_name) as event_names,
    from all_params
