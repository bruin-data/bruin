...
type: bq.sql
description: The events table contains all events and parameters from the Firebase Analytics export. The underlying table is partitioned by date and clustered by event_name. This table is used for ad-hoc analysis and is not used for reporting.
...
select
    ...
    if(
        ...
        (
            select json_object(array_agg(e.id), array_agg(e.value))
    ...
    struct(
        ...
        lax_int64(device.time_zone_offset_seconds)
        / 3600 as device_time_zone_offset,
    ...
    lax_int64(event_params.previous_first_open_count)
