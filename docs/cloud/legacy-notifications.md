...
Legacy notifications are defined in pipeline files. The `notifications` block can be set at the pipeline level in `pipeline.yml`, on an asset, column check, or custom check, or under `default_values`. The connections they reference must exist in [Bruin Cloud Connections](/cloud/connections) first.
...
## Migrate to the new notification system

A team owner or admin can open **Team Settings → Notifications** and click **Switch to new notifications**. Bruin converts existing notification settings in `pipeline.yml` into enabled rules. After migration, notification settings in pipeline files are ignored.

Migration is one-way. It supports pipeline-level Slack, Microsoft Teams, Discord, and email settings.

See [Notifications](/cloud/notifications) to create and manage the new notification rules.
