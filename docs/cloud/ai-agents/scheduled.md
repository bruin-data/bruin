...
A **scheduled agent** is an existing AI agent given a specific task to run on a recurring schedule — daily reports, threshold alerts, custom SQL runs. Each scheduled run uses the agent's connection set, integrations, and CLI access, and posts results to its messaging integrations or the Bruin Cloud chat.
...
Pick a notification channel — Slack, Teams, WhatsApp, or any integration that is already wired up to the underlying agent. Only integrations the agent has access to appear here.
