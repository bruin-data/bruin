...
def test_hoist_declares_no_declare_is_noop():
    from .main import hoist_declares

    result = hoist_declares("SELECT 1; SELECT 2;", "bigquery")
    assert result["error"] == ""
    assert result["query"] == "SELECT 1; SELECT 2;"


def test_hoist_declares_already_ordered_is_noop():
    from .main import hoist_declares

    src = "DECLARE x INT64;\nSELECT 1;"
    result = hoist_declares(src, "bigquery")
    assert result["error"] == ""
    assert result["query"] == src


def test_hoist_declares_reorders_when_needed():
    from .main import hoist_declares

    result = hoist_declares("SET x = 1;\nDECLARE y INT64;\nSELECT 1;", "bigquery")
    assert result["error"] == ""
    # Original statement text is preserved verbatim; only order and separators change.
    assert result["query"] == "DECLARE y INT64;\nSET x = 1;\nSELECT 1;"


def test_hoist_declares_string_literal_not_treated_as_declare():
    from .main import hoist_declares

    src = "SELECT 'declare bankruptcy' AS msg;"
    result = hoist_declares(src, "bigquery")
    assert result["error"] == ""
    assert result["query"] == src


def test_hoist_declares_nested_in_begin_end_is_untouched():
    """DECLARE inside BEGIN..END is not top-level, so the script is not reordered."""
    from .main import hoist_declares
...
def test_hoist_declares_semicolon_inside_string_literal_preserved():
    """The tokenizer must not split inside a string literal."""
    from .main import hoist_declares
...
def test_hoist_declares_escaped_quote_inside_literal():
    from .main import hoist_declares

    src = "SET msg = 'it''s fine';\nDECLARE y INT64;"
    result = hoist_declares(src, "bigquery")
    assert result["error"] == ""
    assert result["query"] == "DECLARE y INT64;\nSET msg = 'it''s fine';"


def test_hoist_declares_preserves_comments_attached_to_declare():
    from .main import hoist_declares

    src = "SET x = 1;\n-- setup\nDECLARE y INT64;"
    result = hoist_declares(src, "bigquery")
    assert result["error"] == ""
    # The leading comment travels with its statement (it's inside the slice).
    assert result["query"] == "-- setup\nDECLARE y INT64;\nSET x = 1;"


def test_hoist_declares_list_preserves_text():
    from .main import hoist_declares_list

    src = ["SET x = 1", "DECLARE y INT64", "SELECT 1"]
    result = hoist_declares_list(src, "bigquery")
    assert result["error"] == ""
    assert result["queries"] == ["DECLARE y INT64", "SET x = 1", "SELECT 1"]


def test_hoist_declares_list_no_declare_is_noop():
    from .main import hoist_declares_list

    src = ["SELECT 1", "SELECT 2"]
    result = hoist_declares_list(src, "bigquery")
    assert result["error"] == ""
    assert result["queries"] == src
