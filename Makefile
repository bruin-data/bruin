...
test-full:
	...
	@$(MAKE) rustsqlparser-lib
	@env SF_DISABLE_MINICORE=true go test -json -tags="no_duckdb_arrow" -race -p "$(TEST_CONCURRENCY)" -vet=off -timeout 10m ./cmd/... ./pkg/... ./templates/...
	@echo "$(OK_COLOR)==> Running the semantic-engine module tests with race detection$(NO_COLOR)"
	@cd semantic-engine && env SF_DISABLE_MINICORE=true go test -json -race -p "$(TEST_CONCURRENCY)" -timeout 10m ./...
