...
func TestCreateTaskFromYamlDefinition(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "requirements file is parsed correctly as a string path",
			args: args{
				filePath: filepath.Join("testdata", "yaml", "task-with-requirements-file", "task.yml"),
			},
			want: &pipeline.Asset{
				ID:   "afa27b44d43b02a9fea41d13cedc2e4016cfcf87c5dbf990e593669aa8ce286d",
				Name: "hello-world",
				...
				ExecutableFile: pipeline.ExecutableFile{
					Name:    "task.yml",
					Path:    path.AbsPathForTests(t, filepath.Join("testdata", "yaml", "task-with-requirements-file", "task.yml")),
					Content: mustRead(t, filepath.Join("testdata", "yaml", "task-with-requirements-file", "task.yml")),
				},
		...
		{
			name: "requirements file in external folder is parsed correctly",
			args: args{
				filePath: filepath.Join("testdata", "yaml", "task-with-external-req-file", "task.yml"),
			},
			want: &pipeline.Asset{
				ID:   "afa27b44d43b02a9fea41d13cedc2e4016cfcf87c5dbf990e593669aa8ce286d",
				Name: "hello-world",
				...
				ExecutableFile: pipeline.ExecutableFile{
					Name:    "task.yml",
					Path:    path.AbsPathForTests(t, filepath.Join("testdata", "yaml", "task-with-external-req-file", "task.yml")),
					Content: mustRead(t, filepath.Join("testdata", "yaml", "task-with-external-req-file", "task.yml")),
				},
				RequirementsFile: "../../../shared/requirements.txt",
				Requirements:     nil,
				Secrets:          []pipeline.SecretMapping{},
				Upstreams:        []pipeline.Upstream{},
				Columns:          []pipeline.Column{},
