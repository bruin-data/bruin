...
func ConvertYamlToTask(content []byte) (*Asset, error) {
	...
	for index, column := range definition.Columns {
		...
		for _, test := range column.Tests {
			...
			tests = append(tests, NewColumnCheck(definition.Name, column.Name, test.Name, ColumnCheckValue(test.Value), test.Blocking, test.Description, notificationsOrNil(test.Notifications)))
		}
