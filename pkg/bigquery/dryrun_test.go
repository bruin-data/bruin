...
func TestDryRunner_DryRun(t *testing.T) {
	...
	t.Run("view materialization does not double-wrap when body is already view DDL", func(t *testing.T) {
		t.Parallel()
		connGetter := &mockConnectionGetter{}
		queryExtractor := &mockQueryExtractor{}

		body := "CREATE OR REPLACE VIEW project.dataset.events AS SELECT 1"
		asset := pipeline.Asset{
			Name:       "project.dataset.events",
			Type:       pipeline.AssetTypeBigqueryQuery,
			Connection: "bq-conn",
		...
		queryExtractor.On("ExtractQueriesFromString", body).Return(
			[]*query.Query{{Query: body}}, nil,
		)
		...
		require.NoError(t, err)
		assert.Equal(t, "CREATE_VIEW", result["bigquery"].(*bigquery.QueryStatistics).StatementType)

		mockDryRunnerQuerier.AssertExpectations(t)
	})

	t.Run("view materialization detects DDL after leading line comment", func(t *testing.T) {
		t.Parallel()
		connGetter := &mockConnectionGetter{}
		queryExtractor := &mockQueryExtractor{}

		body := "-- daily events view\nCREATE OR REPLACE VIEW project.dataset.events AS SELECT 1"
		asset := pipeline.Asset{
			Name:       "project.dataset.events",
			Type:       pipeline.AssetTypeBigqueryQuery,
			Connection: "bq-conn",
		...
		).Return(&bigquery.QueryStatistics{StatementType: "CREATE_VIEW"}, nil)
	...
	t.Run("view materialization detects DDL after leading block comment", func(t *testing.T) {
		t.Parallel()
		connGetter := &mockConnectionGetter{}
		queryExtractor := &mockQueryExtractor{}

		body := "/* banner\n  multi-line */\nCREATE VIEW project.dataset.events AS SELECT 1"
		asset := pipeline.Asset{
			Name:       "project.dataset.events",
			Type:       pipeline.AssetTypeBigqueryQuery,
			Connection: "bq-conn",
