...
func TestClient_QueryDryRun(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			response: map[string]interface{}{
				"error": googleapi.Error{
					Code:    400,
					Message: "Syntax error: Unexpected end of input at [1:15]",
				},
			},
			statusCode:    http.StatusBadRequest,
			expectedError: "Syntax error: Unexpected end of input at [1:15]",
		},
		{
			...
			response: map[string]interface{}{
				"error": googleapi.Error{
					Code:    404,
					Message: "Not found: Table test-project:dataset.nonexistent_table was not found in location US",
				},
			},
			statusCode:    http.StatusNotFound,
			expectedError: "Not found: Table test-project:dataset.nonexistent_table was not found in location US",
		},
		{
			...
			response: map[string]interface{}{
				"error": googleapi.Error{
					Code:    403,
					Message: "Access Denied: BigQuery BigQuery: Permission denied while getting data from: test-project:dataset.restricted_table",
				},
			},
			statusCode:    http.StatusForbidden,
			expectedError: "Access Denied: BigQuery BigQuery: Permission denied while getting data from: test-project:dataset.restricted_table",
		},
		{
			name:  "job status with error",
		...
		},
		{
			...
			statusCode:    http.StatusOK,
			expectedError: "missing job status for dry run",
		},
		{
			name:  "missing statistics in job status",
			query: "SELECT * FROM test_table",
			response: map[string]interface{}{
				"jobReference": map[string]interface{}{
					"jobId": "test-job-id",
			...
			statusCode:    http.StatusOK,
			expectedError: "missing statistics in dry run status",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := d.QueryDryRun(context.Background(), &query.Query{Query: tt.query})
...
func TestClient_QueryDryRun_EdgeCases(t *testing.T) {
	t.Parallel()
	...
	t.Run("empty query", func(t *testing.T) {
		...
		result, err := d.QueryDryRun(context.Background(), &query.Query{Query: ""})
