...
func TestBuildTableExistsQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			tableName: "dataset.table",
			wantQuery: "SELECT 1 FROM `test-project.dataset.INFORMATION_SCHEMA.TABLES` WHERE table_name = 'table'",
			wantErr:   false,
		},
		{
			...
			wantQuery: "SELECT 1 FROM `other-project.dataset.INFORMATION_SCHEMA.TABLES` WHERE table_name = 'table'",
			wantErr:   false,
		},
		{
