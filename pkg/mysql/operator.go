...
func (o BasicOperator) RunTask(ctx context.Context, p *pipeline.Pipeline, t *pipeline.Asset) error {
	...
	if len(queries) > 1 {
		...
		// this is not supported and we should not allow it, MySQL-like platforms accepts multiple SQL statements in a single query, that's what we use instead.
		return errors.New("MySQL-like operators can only handle one query at a time, this seems like a bug, please report the issue to the Bruin team")
	}
...
func (o *MetadataOperator) Run(ctx context.Context, ti scheduler.TaskInstance) error {
	...
	client, ok := o.connection.GetConnection(connName).(MysqlClient)
	if !ok {
		return errors.Errorf("'%s' either does not exist or is not a MySQL connection", connName)
	}
	...
	if err != nil {
		_, _ = writer.Write([]byte("Failed to push metadata to MySQL, skipping...\n"))
		return err
	}
