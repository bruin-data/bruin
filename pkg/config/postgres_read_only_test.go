...
func TestPostgresConnectionReadOnlyRoundTrip(t *testing.T) {
	t.Parallel()
	for _, conn := range []any{&PostgresConnection{}} {
...
func TestPostgresConnectionReadOnlySchema(t *testing.T) {
	t.Parallel()
	...
	for _, name := range []string{"PostgresConnection"} {
