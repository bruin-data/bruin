...
func TestUnmarshalStrict(t *testing.T) {
	...
	t.Run("valid yaml with only known keys succeeds", func(t *testing.T) {
		t.Parallel()
		buf := []byte("name: jane\nmiddle: james\nsurname: doe\nage: 30\nheight: 1.65\nskills: [java, python]\nfamily:\n  parents:\n    parent1: mama\n    parent2: papa\n  siblings: [sister, brother]\n")
		var out exampleData
		err := UnmarshalStrict(buf, &out)
		require.NoError(t, err)
		assert.Equal(t, "jane", out.Name)
		assert.Equal(t, "mama", out.Family.Parents.FirstParent)
	})
	...
	t.Run("empty input is ok", func(t *testing.T) {
		t.Parallel()
		var out exampleData
		err := UnmarshalStrict([]byte(""), &out)
		require.NoError(t, err)
		err = UnmarshalStrict([]byte("   \n"), &out)
		require.NoError(t, err)
	})
}
...
func TestReadYamlStrict(t *testing.T) {
	...
	t.Run("valid file succeeds", func(t *testing.T) {
		...
		fs := afero.NewOsFs()
		var out exampleData
		err := ReadYamlStrict(fs, "testdata/yamlreader/successful-validation.yml", &out)
		require.NoError(t, err)
		assert.Equal(t, "jane", out.Name)
	})

	t.Run("file with unknown key errors", func(t *testing.T) {
		t.Parallel()
		fs := afero.NewMemMapFs()
		content := "name: test\ninvalid_key: value\n"
		require.NoError(t, afero.WriteFile(fs, "/strict.yml", []byte(content), 0o644))
		var out exampleData
		err := ReadYamlStrict(fs, "/strict.yml", &out)
		require.Error(t, err)
