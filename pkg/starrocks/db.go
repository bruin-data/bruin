...
// StarRocksConfig is the driver-level config the client depends on. It is
// implemented by the package-local Config (see config.go).
...
func (c *Client) initializeDB(ctx context.Context) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to connect to StarRocks")
	}
...
func (c *Client) Ping(ctx context.Context) error {
	...
	if err := c.RunQueryWithoutResult(ctx, &q); err != nil {
		return errors.Wrap(err, "failed to run test query on StarRocks connection")
	}
...
`
	...
	if err != nil {
		return nil, fmt.Errorf("failed to query StarRocks information_schema: %w", err)
	}
