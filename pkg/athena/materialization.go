...
func buildSCD2ByColumnQuery(asset *pipeline.Asset, query, location string) ([]string, error) {
	...
	if incrementalKey != "" {
		validFromExpr = fmt.Sprintf("with_timezone(CAST(s.%s AS TIMESTAMP), 'UTC')", incrementalKey)
		validUntilUpdateExpr = fmt.Sprintf("with_timezone(CAST(s.%s AS TIMESTAMP), 'UTC')", incrementalKey)
	}
...
func buildSCD2ByColumnFullRefresh(asset *pipeline.Asset, query, location string) ([]string, error) {
	...
	if asset.Materialization.IncrementalKey != "" {
		validFromExpr = fmt.Sprintf("with_timezone(CAST(src.%s AS TIMESTAMP), 'UTC')", asset.Materialization.IncrementalKey)
	}
...
func buildSCD2ByTimeQuery(asset *pipeline.Asset, query, location string) ([]string, error) {
	...
	changeCondition := fmt.Sprintf("with_timezone(CAST(s.%s AS TIMESTAMP), 'UTC') > t._valid_from", incrementalKey)
	...
	sqlLines := []string{
		...
		"\tt._valid_from,",
		"\t\tCASE",
		fmt.Sprintf("\t\t\tWHEN _matched_by_source IS NOT NULL AND (%s) THEN with_timezone(CAST(s.%s AS TIMESTAMP), 'UTC')", changeCondition, incrementalKey),
		"\t\t\tWHEN _matched_by_source IS NULL THEN (SELECT now FROM time_now)",
		"\t\t\tELSE t._valid_until",
		"\t\tEND AS _valid_until,",
		"\t\tCASE",
		...
		"to_insert AS (",
		fmt.Sprintf("\tSELECT %s,", strings.Join(toInsertSelectCols, ", ")),
		fmt.Sprintf("\twith_timezone(CAST(s.%s AS TIMESTAMP), 'UTC') AS _valid_from,", incrementalKey),
		"\twith_timezone(TIMESTAMP '9999-12-31 23:59:59', 'UTC') AS _valid_until,",
		"\tTRUE AS _is_current",
		"\tFROM source s",
		fmt.Sprintf("\tLEFT JOIN current_data t ON (%s)", joinCondition),
...
func buildSCD2ByTimeFullRefresh(asset *pipeline.Asset, query, location string) ([]string, error) {
	...
	createQuery := fmt.Sprintf(
		"CREATE TABLE %s WITH (table_type='ICEBERG', is_external=false, location='%s/%s'%s) AS\n"+
			"SELECT %s,\n"+
			"with_timezone(CAST(src.%s AS TIMESTAMP), 'UTC') AS _valid_from,\n"+
			"with_timezone(TIMESTAMP '9999-12-31 23:59:59', 'UTC') AS _valid_until,\n"+
			"TRUE AS _is_current\n"+
			"FROM (%s\n"+
			") AS src",
