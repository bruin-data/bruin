...
var matMap = AssetMaterializationMap{
	...
	pipeline.MaterializationTypeTable: {
		...
		pipeline.MaterializationStrategySCD2ByTime:    buildSCD2ByTimeQuery2,
	},
}
...
// buildSCD2ByTimeQuery2 is a cleaner, column-explicit version of buildSCD2ByTimeQuery, following the structure of buildSCD2ByColumnQuery.
func buildSCD2ByTimeQuery2(asset *pipeline.Asset, query, location string) ([]string, error) {
	query = strings.TrimSuffix(query, ";")
	...
	// Only compare the incremental key for changes
	// joinedChangeCond := []string{
	// 	fmt.Sprintf("t_%s != s_%s", incrementalKey, incrementalKey),
	// }
	joinedChangeCondition := fmt.Sprintf("OR CAST(s_%s AS TIMESTAMP) > t._valid_from", incrementalKey)

	unchangedConds := []string{
		fmt.Sprintf("CAST(s_%s AS TIMESTAMP) = _valid_from", incrementalKey),
	}
	unchangedCondition := strings.Join(unchangedConds, " AND ")

	changeCondition := fmt.Sprintf("t.%s != s.%s", incrementalKey, incrementalKey)
	...
	// unchangedSelectCols = append(unchangedSelectCols, "_valid_from", "_valid_until", "_is_current")
	...
	insertSelectCols = append(insertSelectCols, fmt.Sprintf("(SELECT %s FROM time_now) AS _valid_from", "now"))
...
unchanged AS (
  ...
  FROM joined
  WHERE %s IS NOT NULL 
  AND CAST(s_%s AS TIMESTAMP) = _valid_from
),
-- Rows that need to be expired (changed or missing in source)
to_expire AS (
  SELECT %s,
  ...
  FROM joined
  WHERE %s IS NULL 
  OR CAST(s_%s AS TIMESTAMP) > t._valid_from
),
-- New/changed inserts from source
to_insert AS (
  SELECT %s,
  ...
  LEFT JOIN target t ON %s
  WHERE %s IS NULL 
  OR CAST(s_%s AS TIMESTAMP) > _valid_from	
),
-- Already expired historical rows (untouched)
historical AS (
  SELECT %s
