		...
		},
		{
			...
			want: []string{
				"CREATE TABLE __bruin_tmp_abcefghi WITH (table_type='ICEBERG', is_external=false, location='s3://bucket/__bruin_tmp_abcefghi') AS\n" +
					...
					"\tt._valid_from,\n" +
					"\t\tCASE\n" +
					"\t\t\tWHEN _matched_by_source IS NOT NULL AND (t.col1 != s.col1 OR t.updated_at != s.updated_at) THEN CAST(s.updated_at AS TIMESTAMP WITH TIME ZONE)\n" +
					"\t\t\tWHEN _matched_by_source IS NULL THEN (SELECT now FROM time_now)\n" +
					"\t\t\tELSE t._valid_until\n" +
					"\t\tEND AS _valid_until,\n" +
					"\t\tCASE\n" +
					...
					"\tCAST(s.updated_at AS TIMESTAMP WITH TIME ZONE) AS _valid_from,\n" +
					"\tTIMESTAMP '9999-12-31 23:59:59 UTC' AS _valid_until,\n" +
					"\tTRUE AS _is_current\n" +
					"\tFROM source s\n" +
					"\tLEFT JOIN current_data t ON (t.id = s.id)\n" +
		...
		},
		{
			...
			want: []string{
				"CREATE TABLE __bruin_tmp_abcefghi WITH (table_type='ICEBERG', is_external=false, location='s3://bucket/__bruin_tmp_abcefghi', partitioning = ARRAY['_valid_from']) AS\n" +
					...
					"CAST(src.updated_at AS TIMESTAMP WITH TIME ZONE) AS _valid_from,\n" +
					"TIMESTAMP '9999-12-31 23:59:59 UTC' AS _valid_until,\n" +
					"TRUE AS _is_current\n" +
		...
		},
		{
			...
			want: []string{
				"CREATE TABLE __bruin_tmp_abcefghi WITH (table_type='ICEBERG', is_external=false, location='s3://bucket/__bruin_tmp_abcefghi', partitioning = ARRAY['id']) AS\n" +
					...
					"\tt._valid_from,\n" +
					"\t\tCASE\n" +
					"\t\t\tWHEN _matched_by_source IS NOT NULL AND (CAST(s.ts AS TIMESTAMP WITH TIME ZONE) > t._valid_from) THEN CAST(s.ts AS TIMESTAMP WITH TIME ZONE)\n" +
					"\t\t\tWHEN _matched_by_source IS NULL THEN (SELECT now FROM time_now)\n" +
					"\t\t\tELSE t._valid_until\n" +
					"\t\tEND AS _valid_until,\n" +
					"\t\tCASE\n" +
					"\t\t\tWHEN _matched_by_source IS NOT NULL AND (CAST(s.ts AS TIMESTAMP WITH TIME ZONE) > t._valid_from) THEN FALSE\n" +
					"\t\t\tWHEN _matched_by_source IS NULL THEN FALSE\n" +
					"\t\t\tELSE t._is_current\n" +
					"\t\tEND AS _is_current\n" +
					"\tFROM target t\n" +
					...
					"\tCAST(s.ts AS TIMESTAMP WITH TIME ZONE) AS _valid_from,\n" +
					"\tTIMESTAMP '9999-12-31 23:59:59 UTC' AS _valid_until,\n" +
					"\tTRUE AS _is_current\n" +
					"\tFROM source s\n" +
					"\tLEFT JOIN current_data t ON (t.id = s.id)\n" +
					"\tWHERE (_matched_by_target IS NULL) OR (CAST(s.ts AS TIMESTAMP WITH TIME ZONE) > t._valid_from)\n" +
					")\n" +
...

		{
			...
			want: []string{
				"CREATE TABLE __bruin_tmp_abcefghi WITH (table_type='ICEBERG', is_external=false, location='s3://bucket/__bruin_tmp_abcefghi', partitioning = ARRAY['_valid_from']) AS\n" +
					...
					"CAST(src.ts AS TIMESTAMP WITH TIME ZONE) AS _valid_from,\n" +
					"TIMESTAMP '9999-12-31 23:59:59 UTC' AS _valid_until,\n" +
					"TRUE AS _is_current\n" +
					"FROM (SELECT id, event_name, ts from source_table\n" +
					") AS src",
