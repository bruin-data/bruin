...
func (m *mockConnectionFetcher) GetDuckDBConnection(name string) (DuckDBClient, error) {
	args := m.Called(name)
	get := args.Get(0)
	if get == nil {
		return nil, args.Error(1)
	...
	return get.(DuckDBClient), args.Error(1)
}
...
func TestPatternCheck_Check(t *testing.T) {
	...
	runTestsForCountZeroCheck(
		...
		"SELECT count(*) FROM dataset.test_asset WHERE test_column !~ '(a|b)'",
