...
func TestDB_BuildTableExistsQuery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			tableName:   "a.b.c.d",
			wantErr:     true,
			errContains: "must be in format `schema.table` or `catalog.schema.table`",
		},
		{
		...
		{
			name:      "valid catalog.schema.table format scopes information_schema to the catalog",
			c:         &DB{config: &Config{Catalog: "test_db"}},
			tableName: "mycat.test_schema.test_table",
			wantQuery: "SELECT COUNT(*) FROM mycat.information_schema.tables WHERE table_schema = 'test_schema' AND table_name = 'test_table'",
			wantErr:   false,
		},
		{
