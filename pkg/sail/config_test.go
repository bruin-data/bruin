...
func TestConfig_ToDSN(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "no auth (default Sail server)",
			config: Config{
				Host: "127.0.0.1",
				Port: 32010,
			},
			expected: "uri=grpc+tcp://127.0.0.1:32010",
		},
		{
			...
			expected: "uri=grpc+tcp://sail.example.com:32010;username=admin;password=secret",
		},
		{
		...
		},
		{
			...
			expected: "uri=grpc+tls://sail.example.com:443;adbc.flight.sql.authorization_header=Bearer pat123",
		},
		{
