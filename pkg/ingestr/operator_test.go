...
func TestBasicOperator_CDCMode(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "CDC stream flush parameters override the generic ones",
			asset: &pipeline.Asset{
				Name:       "cdc-asset-stream-flush",
				Connection: "bq",
				Parameters: pipeline.ParameterMap{
					"source_connection":         "pg",
					"source_table":              "public.users",
		...
		{
			name: "CDC stream parameters are ignored without cdc",
			asset: &pipeline.Asset{
				Name:       "non-cdc-asset-stream-params",
				Connection: "bq",
				Parameters: pipeline.ParameterMap{
					"source_connection":         "pg",
					"source_table":              "public.users",
			...
			want: []string{
				"ingest",
				"--source-uri", "postgresql://user:pass@localhost:5432/db",
				"--source-table", "public.users",
				"--dest-uri", "bigquery://uri-here",
				"--dest-table", "non-cdc-asset-stream-params",
				"--yes",
				"--progress", "log",
			},
		},
