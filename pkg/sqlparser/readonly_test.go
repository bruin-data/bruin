...
func TestSQLParserIsReadOnlyQuery(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "system function in normalized argument", query: "SELECT DATEADD(SYSTEM$CANCEL_QUERY('id'), 1, CURRENT_DATE())"},
		{name: "qualified system function", query: "SELECT db.schema.SYSTEM$CANCEL_QUERY('id')"},
		{name: "quoted system function", query: `SELECT "SYSTEM$CANCEL_QUERY"('id')`},
		{name: "system function with comment", query: "SELECT SYSTEM$CANCEL_QUERY /* hidden */ ('id')"},
		{name: "system function in explain", query: "EXPLAIN SELECT SYSTEM$CANCEL_QUERY('id')"},
		{name: "system function sharing alias", query: "WITH SYSTEM$CANCEL_QUERY(x) AS (SELECT 1) SELECT SYSTEM$CANCEL_QUERY('id')"},
		{name: "dynamic SQL function", query: "SELECT * FROM TABLE(TO_QUERY('SELECT 1'))"},
		{name: "system function in dynamic SQL", query: "SELECT * FROM TABLE(TO_QUERY('SELECT SYSTEM$CANCEL_QUERY(''id'')'))"},
		{name: "quoted dynamic SQL function", query: `SELECT * FROM TABLE("TO_QUERY"('SELECT 1'))`},
		{name: "AI evaluation control function", query: "SELECT EXECUTE_AI_EVALUATION('id')"},
...
func TestValidateReadOnlyQueryAlwaysUsesPython(t *testing.T) {
	...
	require.ErrorContains(t, ValidateReadOnlyQuery("SELECT SYSTEM$CANCEL_QUERY('id')", "snowflake"), "read-only")
