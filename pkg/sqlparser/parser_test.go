...
func TestSqlParser_HoistDeclares(t *testing.T) {
	...
	t.Run("declare after non-declare gets hoisted with original text preserved", func(t *testing.T) {
		...
		got, err := parser.HoistDeclares(in, pipeline.AssetTypeBigqueryQuery)
		require.NoError(t, err)
		// Each statement's original text is preserved verbatim (no sqlglot
		// regeneration). Only the order and the separators change.
	...
	t.Run("semicolon inside string literal does not split", func(t *testing.T) {
		...
		got, err := parser.HoistDeclares(in, pipeline.AssetTypeBigqueryQuery)
		require.NoError(t, err)
		// The literal's ';' is not a top-level boundary; SET text preserved verbatim.
		require.Equal(t, "DECLARE y INT64;\nSET separator = ';';", got)
	})
	...
	t.Run("declare inside BEGIN..END block is not hoisted", func(t *testing.T) {
		...
		got, err := parser.HoistDeclares(in, pipeline.AssetTypeBigqueryQuery)
		require.NoError(t, err)
		// The nested DECLARE is not top-level; no reorder, input unchanged.
		require.Equal(t, in, got)
	})
	...
	t.Run("array type syntax preserved verbatim", func(t *testing.T) {
		// BigQuery `array<STRING>` is a syntax sqlglot might reformat on
		// regenerate (e.g. to `ARRAY<STRING>`). Since we slice the original
		// text instead of regenerating, the casing must survive.
