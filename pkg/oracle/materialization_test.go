...
USING (
  ...
  WHERE t1.bruin_valid_from < CAST(s1.ts AS TIMESTAMP(6) WITH TIME ZONE) AT TIME ZONE 'UTC' AND t1.bruin_is_current = 1
) source
ON ((target.id = source.id OR (target.id IS NULL AND source.id IS NULL)) AND source.bruin_is_current_src = 1)
WHEN MATCHED THEN
  UPDATE SET
    target.bruin_valid_until = CAST(source.ts AS TIMESTAMP(6) WITH TIME ZONE) AT TIME ZONE 'UTC',
    target.bruin_is_current  = 0
  WHERE target.bruin_valid_from < CAST(source.ts AS TIMESTAMP(6) WITH TIME ZONE) AT TIME ZONE 'UTC'
WHEN NOT MATCHED THEN
  INSERT (id, event_name, ts, bruin_valid_from, bruin_valid_until, bruin_is_current)
  VALUES (source.id, source.event_name, source.ts, CAST(source.ts AS TIMESTAMP(6) WITH TIME ZONE) AT TIME ZONE 'UTC', FROM_TZ(CAST(TO_TIMESTAMP('9999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS') AS TIMESTAMP(6)), 'UTC'), 1);
END;`,
		},
		{
