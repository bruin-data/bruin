...
func buildSCD2QueryByTime(asset *pipeline.Asset, query string) (string, error) {
	...
	insertValues = append(insertValues,
		"TRY_CAST(source."+asset.Materialization.IncrementalKey+" AS TIMESTAMP)",
...
USING (
  ...
  WHERE  t1._valid_from < TRY_CAST(s1.%s AS TIMESTAMP) AND t1._is_current
) AS source
ON  %s

WHEN MATCHED AND (
  target._valid_from < TRY_CAST(source.%s AS TIMESTAMP)
) THEN
  UPDATE SET
    target._valid_until = TRY_CAST(source.%s AS TIMESTAMP),
...
func buildSCD2ByColumnQuery(asset *pipeline.Asset, query string) (string, error) {
	...
	insertCols = append(insertCols, "_valid_from", "_valid_until", "_is_current")
	insertValues = append(insertValues, "CURRENT_TIMESTAMP()", "TO_TIMESTAMP('9999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS')", "TRUE")
	pkList := strings.Join(primaryKeys, ", ")
...
SELECT
  src.*,
  TRY_CAST(%s AS TIMESTAMP) AS _valid_from,
