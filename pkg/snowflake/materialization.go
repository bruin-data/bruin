...
// buildPKConditions creates primary key join conditions for two table aliases
...
func buildSCD2ByTimefullRefresh(asset *pipeline.Asset, query string) (string, error) {
	...
	selectColumns = append(selectColumns, fmt.Sprintf("CAST(src.%s AS TIMESTAMP)", asset.Materialization.IncrementalKey), "TO_TIMESTAMP('9999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS')", "TRUE")
