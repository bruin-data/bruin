...
func (m *mockSQLParser) ExtractColumns(sql, dialect string) ([]string, error) {
	args := m.Called(sql, dialect)
	return args.Get(0).([]string), args.Error(1)
}
...
func TestValidateColumnsMatchQuery(t *testing.T) {
	t.Parallel()

	// Create a mock SQL parser
	parser := new(mockSQLParser)

	// Create the new rule
	rule := ColumnMatchQueryValidatorRule{
		parser:   parser,
	...
	tests := []struct {
		...
		{
			name: "SQL asset with matching columns",
			asset: &pipeline.Asset{
				Type: pipeline.AssetTypeBigqueryQuery,
				ExecutableFile: pipeline.ExecutableFile{
		...
		{
			name: "SQL asset with missing columns in YAML",
			asset: &pipeline.Asset{
				Type: pipeline.AssetTypeBigqueryQuery,
				ExecutableFile: pipeline.ExecutableFile{
					Content: "SELECT id, name, email FROM users",
			...
			expectedIssues: 1,
			description:    "Should warn about missing 'email' column in YAML",
		},
		{
			name: "SQL asset with no columns defined in YAML",
			asset: &pipeline.Asset{
				Type: pipeline.AssetTypeBigqueryQuery,
				ExecutableFile: pipeline.ExecutableFile{
					Content: "SELECT id, name, email FROM users",
			...
			expectedIssues: 1,
			description:    "Should warn about all columns missing from YAML when none are defined",
		},
		{
			name: "Non-SQL asset should be skipped",
			asset: &pipeline.Asset{
				Type: pipeline.AssetTypePython,
				Columns: []pipeline.Column{
					{Name: "id"},
		...
		{
			name: "Asset without columns should be skipped",
			asset: &pipeline.Asset{
				Type: pipeline.AssetTypeBigqueryQuery,
				ExecutableFile: pipeline.ExecutableFile{
			...
			setup: func(p *mockSQLParser) {
				p.On("ExtractColumns", "SELECT id, name FROM users", "bigquery").Return([]string{}, nil)
			},
			expectedIssues: 0,
			description:    "Should skip when no columns found in query",
		},
	}
	...
	for _, tt := range tests { //nolint
		t.Run(tt.name, func(t *testing.T) {
			...
			parser.ExpectedCalls = nil
			if tt.setup != nil {
				tt.setup(parser)
			}

			issues, err := rule.ValidateAsset(context.Background(), nil, tt.asset)
			require.NoError(t, err)
