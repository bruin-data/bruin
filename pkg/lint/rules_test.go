...
func TestValidateInvalidPythonModuleName(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			asset: &pipeline.Asset{
				Type: pipeline.AssetTypePython,
				ExecutableFile: pipeline.ExecutableFile{
					Path: "test/assets/valid_module/hello.oy",
				},
			},
			want:    nil,
			wantErr: false,
		...
		{
			name: "module name with hyphen",
			asset: &pipeline.Asset{
				Type: pipeline.AssetTypePython,
				ExecutableFile: pipeline.ExecutableFile{
					Path: "test/assets/invalid-module/hello.py",
				},
			},
			want: []*Issue{
				{
					...
					Description: "Invalid Python module name 'test/assets/invalid-module/hello.py' for asset ''. Directory names cannot contain hyphens ('-') as they cannot be imported in Python.",
				},
			},
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := ValidateInvalidPythonModuleName(context.Background(), &pipeline.Pipeline{}, tt.asset)
			fmt.Println(err)
			fmt.Println(got)
			if tt.wantErr {
				require.Error(t, err)
