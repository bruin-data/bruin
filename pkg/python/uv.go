...
func (u *UvChecker) installUvCommand(ctx context.Context, dest string) error {
	...
	if runtime.GOOS == "windows" {
		...
		commandInstance = exec.Command(fmt.Sprintf(" set -o pipefail; curl -LsSf https://astral.sh/uv/%s/install.sh | UV_INSTALL_DIR=\"%s\" NO_MODIFY_PATH=1 /bin/sh", UvVersion, dest)) //nolint:gosec
	}
